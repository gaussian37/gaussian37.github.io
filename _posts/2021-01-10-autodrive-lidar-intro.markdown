...
- 다음 코드는 1차식 즉, 평면과 2차식 즉, 2차 곡면을 동일한 `RANSAC` 방식으로 추정하는 코드입니다. 어떤 면을 선택해야 하는 지는 다양한 기준을 실험해 보면서 선택할 수 있습니다. 아래는 단순히 `inlier` 갯수를 이용하여 모델링한 것입니다. 함수의 출력은 앞의 `pcd.segment_plane`과 호환이 되도록 구성하였습니다.
