...
- `카메라 좌표계` 기준의 `구면 투영법`은 카메라가 바라보는 방향과 3차원 좌표계의 좌표축이 동일하다는 관점에서 `구면 투영법`을 진행합니다. 반면 `World 좌표계` 기준의 `구면 투영법`은 `World 좌표계`의 좌표축을 기준으로 `구면 투영법`을 진행하기 때문에 카메라와 `World 좌표계` 간의 `Extrinsic` 관계를 고려해야 합니다. 이 방법을 이용하면 `World 좌표계` 기준으로 여러개의 카메라를 `구면 투영`할 수 있고 각 카메라의 기준이 동일한 하나의 `World 좌표계`이기 때문에 파노라마 뷰를 생성할 수 있습니다.
- 따라서 `카메라 좌표계` 기준의 `구면 투영법`은 카메라의 `Intrinsic`만 사용하는 반면 `World 좌표계` 기준의 `구면 투영법`은 카메라의 `Intrinsic`, `Extrinsic`을 사용합니다.
...
- 따라서 이 글의 마지막에서는 4개의 카메라를 이용하여 구면 투영법을 통해 파노라마 이미지를 생성하는 방법에 대하여 다루어 보도록 하겠습니다.
...
- `RDF` 좌표축에서의 구면 좌표계는 위 그림과 같이 정의될 수 있습니다. 이 좌표계를 이용하여 이미지를 투영하면 **이미지의 중심**은 **좌표계의 원점**에 대응될 수 있습니다.
- 2D 이미지에 투영해야 하므로 $$ r $$ 값은 `normalization`을 통하여 1로 처리합니다. 이것은 3D 좌표를 이미지에 투영할 때, $$ z $$ 값인 `depth`를 `normalization`하는 것에 대응됩니다.
- 이 과정을 통하여 구면 투영법을 통해 얻는 이미지 각 픽셀의 $$ (u, v) $$ 좌표는 $$ (\phi, \theta) $$ 의 의미를 가집니다.

<br>
<center><img src="../assets/img/vision/concept/spherical_projection/4.png" alt="Drawing" style="width: 400px;"/></center>
<br>

- 위 그림을 살펴보면 이미지의 중점에서 $$ \phi = 0, \theta = 0 $$ 임을 알 수 있습니다. $$ u $$ 가 증가하는 방향 (이미지의 오른쪽 방향)으로 $$ \phi $$ 가 증가하고 $$ v $$ 가 증가하는 방향 (이미지의 아래 방향)으로 $$ \theta $$ 가 증가합니다.
...
<br>
<center><img src="../assets/img/vision/concept/spherical_projection/5.png" alt="Drawing" style="width: 600px;"/></center>
<br>
