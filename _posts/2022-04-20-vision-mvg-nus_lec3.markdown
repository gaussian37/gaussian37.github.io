...
- ① `line at infinity` 와 `circular points` 개념을 배우고 이 개념을 이용하여 `affine` 또는 `projective distortion`을 제거하는 방법에 대하여 배워보도록 하겠습니다.
...
- ① **$$ C^{*}_{\infty} $$ 은 4 DoF (Degree of Freedom)을 가집니다.** 원래 $$ C^{*}_{\infty} $$ 는 3 X 3 대칭 행렬이고 (3, 3) 위치의 스케일 값을 무시한다고 하면 5개의 자유도를 가집니다. 그리고 $$ \text{det}(C^{*}_{\infty}) = 0 $$ 을 만족해야 하므로 자유 변수가 1개 더 생기므로 DoF는 4개로 줄어듭니다.
- 여기서 $$ \text{det}(C^{*}_{\infty}) = 0 $$ 를 만족할 때, `DoF`가 1 감소하는 것은 다음 예시를 통해 이해할 수 있습니다.
...
- 위 행렬의 기본적인 `DoF`는 3입니다. 여기서 (2, 2) 위치의 값이 스케일이라면 (`Up to Scale`) 앞에서 다룬 내용과 같이 `DoF`가 2가 됩니다.
- 만약 $$ \text{det}(S) = 0 $$ 을 만족한다면 $$ ad - b^{2} = 0 $$ 을 만족해야 하고 $$ a $$ 가 정해지면 나머지 $$ d $$ 의 값에 `dependency`가 발생하기 때문에 `DoF`가 1로 줄어들게 됩니다.
- 이와 같은 이유로 `determinant = 0`인 상황에서는 `DoF`가 1 감소하게 됩니다.
