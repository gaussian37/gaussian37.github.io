...
- 위 식에서 **② 전제 조건**인 `small motion`을 적용해 보겠습니다. 즉, $$ dy, dx $$ 가 굉장히 작은 값을 가진다는 것이 `small motion`을 의미합니다. 인접한 두 영상의 시간 차이 $$ dt $$가 충분히 작아서 `small motion` 조건을 만족한다면 테일러 급수에 따라 다음과 같이 식을 정리할 수 있습니다.
