...
- 리눅스 사용 시 비밀번호를 변경하려면 `passwd` 명령어를 통해 변경하면 됩니다.
- 만약 비밀번호의 패턴이 너무 간단하거나 원하는 대로 변경되지 않는 다면 다음 명령어를 이용해야 합니다.
    - `sudo passwd 사용자이름`
- 위 명령어는 관리자 권한으로 현재 사용자 이름에 해당하는 비밀번호를 어떤 패턴 상관없이 변경할 수 있도록 해줍니다.
- 여기서 조심해야 할 것은 `sudo su` 또는 단순히 `sudo passwd`를 이용하여 비밀번호를 변경하면 **관리자의 비밀번호가 변경되므로** 현재 사용자(ex. /home/me 인 경우 me)의 비밀번호가 변경되지 않습니다.
