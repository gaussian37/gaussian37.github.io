...
- 위 물체가 자동차라고 하였을 때, 가속도를 주기 위해 엑셀을 밟아야 하는데 정확히 $$ u(k) $$ 만큼의 가속도를 줄 정도로 엑셀을 밟는 것은 사실 매우 어렵습니다. 예를 들어 내가 5m 이동하겠다고 마음 먹고 이동하는데, 정확히 5m 이동 하기는 어렵습니다. 5m에 최대한 가깝게 이동하려고 한다고 하더라도 오차가 발생하게 되는데 이 오차가 노이즈 입니다. 따라서 이런 `제어 입력`에는 `noise`가 발생하는 데 이 노이즈를 고려해 주어야 하기 때문에 $$ w(k) $$를 같이 고려해 주어야 합니다.
